...
3. **Surgical Changes:** Modify _only_ what is necessary for the current task. Do not reformat unrelated code or refactor neighboring functions unless explicitly requested. Clean up dead code _only_ if your changes created it.
...
_Note: A formal test runner is not yet configured in `package.json`, but when implemented (e.g., Vitest or Jest), adhere to these standards:_
...
- **Test Execution:** Before committing any significant logic changes, agents _must_ verify changes by running tests.
...
- **Separation of Concerns:**
  ...
  - `src/renderer/`: No Node.js built-in modules (`fs`, `path`, `child_process`) or Electron modules. All system interactions _must_ go through the `window.api` bridge defined in `preload.ts`.
